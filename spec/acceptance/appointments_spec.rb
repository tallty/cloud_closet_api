...
resource "预约存入衣服到我的衣橱" do
  ...
  #   parameter :number_alias, "预约数量", require: true, scope: :appointment
  ...
  #   let(:number_alias) {appointment_attrs[:number_alias]}
  ...
  post 'appointments/:id/pay_by_balance' do
    ...
    describe '由管理员创建的订单，支付后 状态直接切换为 storing' do
      before do
        @appointment.update(created_by_admin: true)
      end
      let(:id) { @appointment.id }
      
      example "用户使用余额支付成功（ 由管理员创建的订单" do
        do_request
        puts response_body
        expect(Appointment.find(@appointment.id).aasm_state).to eq('storing')
        expect(status).to eq(201)
      end
    end
  end
